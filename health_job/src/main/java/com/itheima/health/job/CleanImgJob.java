...
public class CleanImgJob {
    ...
    public void cleanImg(){
        ...
        log.debug ("imgInDb有{}张图片，",imgInDb.size ());
        //七牛-数据库的
        imgIn7Niu.removeAll (imgInDb);
        log.debug ("有{}张图片需要删除，",imgInDb.size ());
        //要删除的
        try {
            log.info ("开始删除图片",imgIn7Niu.size ());
            QiNiuUtils.removeFiles (imgIn7Niu.toArray (new String[]{}));
            log.info ("删除图片成功",imgIn7Niu.size ());
        } catch (Exception e) {
            //e.printStackTrace ( );
            log.info ("清理垃圾图片失败",e);
        }
